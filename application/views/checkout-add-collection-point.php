      ...
      <?php if($product_is_in_donate == 'N'): ?>
        ...
        <div class="form__radios">
          <div class="form__radio">
            ...
            <input checked id="ship" value="ship" name="shipping_method" type="radio" />
          </div>
          <div class="form__radio">
            <label for="visa"><svg class="icon">
                <use xlink:href="#icon-visa" />
              </svg>Pickup From Our  Office</label>
            <input id="pickup" value="pickup" name="shipping_method" type="radio" />
          </div>
        </div>
        <div id="shippingAddressDiv" style="margin-top: 10px;">
          <h2>Address</h2>
          <div class="card">
            <?php if($dilivertAddress): ?>
              <select name="address" id="address" style="width: 100%;">
                <?php foreach ($dilivertAddress as $key=> $da) { ?>
                  <option value="<?=$da['id']?>_____<?=$da['village']?>, <?=$da['street']?>, <?=$da['city']?> <?=$da['country']?>"> <?=$da['village']?> <?=$da['street']?> <?=$da['area']?> <?=$da['city']?> <?=$da['country']?></option>
                <?php } ?>
              </select>
            <?php else: ?>
              <input type="hidden" name="address" id="address" value="N">
              Addreaa Not available. <a href="<?php echo base_url();?>add-dilivery-address"> click here</a> to add address
            <?php endif; ?>
          </div>
        </div>
        <script>
          $(document).ready(function(){
            $('input[type=radio][name=shipping_method]').change(function() {
              var system_shipping_charge  =   parseFloat('<?php echo SHIPPING_CHARGE; ?>');
              var finalprice              =   parseFloat('<?php echo $finalprice; ?>');

              var finaltotal              =   0.00;
              var shipping_charge         =   0.00;
              var inclusice_of_vat        =   0.00;
              var subtotal                =   0.00;
              var vat_amount              =   0.00;

              if (this.value == 'ship') {
                $("#shippingAddressDiv").css("display","block");
                shipping_charge           =   system_shipping_charge;
                finaltotal                =   finalprice;
                inclusice_of_vat          =   (finaltotal+shipping_charge);
                vat_amount                =   (finaltotal*5/100);
                subtotal                  =   (inclusice_of_vat-vat_amount);
              } else {
              ...
              $('#shipping_charge_html').html('AED'+shipping_charge.toFixed(2));
              $('#inclusice_of_vat').val(inclusice_of_vat.toFixed(2));
              $('#inclusice_of_vat_html').html('AED'+inclusice_of_vat.toFixed(2));
              $('#subtotal').val(subtotal.toFixed(2));
              $('#subtotal_html').html('AED'+subtotal.toFixed(2));
              $('#vat_amount').val(vat_amount.toFixed(2));
              $('#vat_amount_html').html('AED'+vat_amount.toFixed(2));
            });
          });
        </script>
      <?php endif; ?>
    ...
    <fieldset>
      ...
      <div class="form__radios">
        <div class="form__radio">
          <label for="visa"><svg class="icon">
              <use xlink:href="#icon-visa" />
            </svg>Credit/Debit Card</label>
          <input checked id="stripe" value="stripe" name="payment_method" type="radio" />
        </div>

        <div class="form__radio">
          <label for="paypal"><svg class="icon">
            <img src="<?=base_url('assets/fav-icon.png')?>" style="width: 4%;margin-left: -13%;" alt="paypal">
      ...
      <table>
        <tbody>
          <tr>
            ...
            <td align="right" id="shipping_charge_html">AED<?php echo number_format($shipping,2); ?></td>
          </tr>
		  </table>
        </tbody>
        ...
        <tbody>
          ...
          <tr style="border-top: 1px solid #b4b4b4;">
            ...
            <td align="right" style="padding: 17px 0px;" id="subtotal_html">AED<?php echo number_format($subTotal,2); ?></td> 
          </tr>
          <tr style="border-top: 1px solid #b4b4b4;">
            <td style="padding: 17px 0px;">VAT</td>
            <input type="hidden" name="vat_amount" id="vat_amount" value="<?php echo number_format($totalVat,2); ?>">
            <td align="right"style="padding: 17px 0px;" id="vat_amount_html">AED<?php echo number_format($totalVat,2); ?></td>
          </tr>
        </tbody>
      </table>
    </div>
