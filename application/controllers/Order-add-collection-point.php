			...
			if($stockCheck == 0){
				...
				if($this->input->post('product_is_donate') == 'N' && $this->input->post('shipping_method') == 'ship' && $this->input->post('address') == 'N'):
					...
					if($this->input->post('payment_method') == 'arabianpoint'){
						...
						if($this->input->post('inclusice_of_vat') > $data['ToatlPoints']){
						...
						if($this->input->post('product_is_donate') == 'N'):
				        	...
				        	if($this->input->post('shipping_method') == 'ship'):
				        		$shippingAddress			=	explode('_____', $this->input->post('address'));
						        $curr_shipping_address_id 	=	$shippingAddress[0];
						        $curr_shipping_address 		=	$shippingAddress[1];
						    else:
			...
			if($stockCheck == 0){
				...
				if($this->form_validation->run() && $error == 'NO'):
				    ...
				    if($orderInsertID):
						...
						if($ORparam["product_is_donate"] == 'N' && $ORparam["shipping_method"] == 'ship'):
							$deliveryAddress		=	$this->geneal_model->getDataByParticularField('da_diliveryAddress','id',(int)$this->session->userdata('payment_shipping_address_id')); 
							$paymentData['address'] =	[
						...
						if($stripeReturnData['status'] == 'success' && $stripeReturnData['stripeChargeId']):
							...
							$data['card_number_error'] 		=	$stripeReturnData['message'];
						endif;
					endif;
				endif;
			}
